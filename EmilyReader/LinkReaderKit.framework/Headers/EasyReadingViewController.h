...
/**
 This view controller is preconfigured with a camera session preview and handles detection and payoff presentation without requiring input from the host application. If you wish to have finer-grain control, use your own view controller and configure LRManager + LRCaptureManager as described in the overview documentation.
 
 See LRManager `+easyReadingViewControllerWithID:secret:delegate`

 

 EasyReadingViewController is intended to be a simple plug-n-play solution to scanning and payoff display. If you need finer-grained control over interaction with the SDK, see LRManager documentation.
...
/**
 ...
 @param failure block performed when authentication has failed
  ...
  @since 1.0
 */
-(void)reauthenticateSuccess:(void (^)(void))success failure:(void (^)(NSError * error))failure;
