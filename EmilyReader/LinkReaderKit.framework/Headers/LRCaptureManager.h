...
/**
 If the AVCaptureOutput contains AVMetadataObjects (eg, Qrcode), they need processing and resolving.

 @param metadataObjects Any number of AVMetadataObject from the capture session.

 @since 1.0
 */
- (void)didFindMetadataObjects:(NSArray *) metadataObjects;
...
/**
 ...
 @discussion
 The LRManager must have successfully completed authorization before scanning can begin. Should authorization fail, scanning will not start and NO will be returned, along with an error message via the -scannerFailedError: method
